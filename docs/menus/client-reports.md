...
> `/mobile`의 화주자료 화면도 이 메뉴의 데이터, 권한, 상태 전이, 서버 액션을 공유한다. 모바일 전용 레이아웃 계약은 [mobile-app.md](./mobile-app.md)를 함께 확인한다.
...
7. `filterClientReportSearchItems`로 일치한 개별 항목만 남긴다.
8. 최대 목록 조회는 현재 300건이며 불필요하게 늘리지 않는다.
...
- 기간 검색 범위가 선택 주차를 벗어나도 편집 폼에는 선택 주차의 기존 보고서가 로드되어야 한다. 빈 폼으로 뜨면 같은 화주·주차 자료를 중복 생성하거나 덮어쓸 수 있다.
