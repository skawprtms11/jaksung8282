...
부서명은 앞뒤 공백과 대소문자를 무시하고 중복 등록할 수 없다. 서버 액션이 저장 전에 명확한 안내를 반환하며, 동시에 같은 이름을 등록하는 경우도 DB 트리거와 트랜잭션 advisory lock이 최종 차단한다. 기존 중복 데이터의 명칭을 바꾸지 않은 일반 수정은 허용하되, 새 등록과 명칭 변경에는 중복 규칙을 적용한다.
