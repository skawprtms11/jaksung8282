...
- 헤더에서 부서와 화주를 선택한다. 화주 미선택 시 현재 구현은 해당 부서의 기본 화주를 검토 필터로 사용할 수 있다.
...
탭은 URL이 아니라 `DepartmentSubmissionEditor`의 로컬 상태다.
...
주차 선택 시 `loadDepartmentSubmissionAction`으로 같은 컴포넌트 안에서 부서 제출자료를 다시 읽고, 공실 탭은 별도 `loadDepartmentVacancyDataAction`으로 갱신한다. URL 초기값과 클라이언트 선택 주차가 어긋나지 않게 유지한다.

## 4. 최초 조회 흐름

1. 현재 또는 URL 주차와 역할별 기본 부서·화주를 결정한다.
2. 서버 세션 클라이언트를 우선 사용하고, 명시적 범위 검증이 있는 전체 조회는 서버 관리자 클라이언트를 제한적으로 사용한다.
3. 부서, 업무구분, 화주 보고서, 등록 화주 수, 현재 부서 제출, 공휴일 화주 옵션, 근무자 옵션, 센터, 월 물동량, 관리자 ID, 미종결 요청을 병렬 조회한다.
4. 화주 검토 목록은 기간 검색이 없으면 선택 주차, 기간 검색이 있으면 선택 주차를 포함한 기간으로 조회한다.
5. 검색 중에는 화주 보고서와 부서 공통사항 모두에서 일치한 개별 항목만 남긴다.
6. 현재 주차 보고서만 요약 카드의 작성현황, 중요도, 물동량 합계에 사용한다.
...
1. 작성현황: 부서 등록 화주 수 대비 현재 주차 작성 보고서 수
2. 내용건수: 현재 주차 화주 항목을 중요도별 집계
3. 물동량: 현재 주차 입고·출고 합계
...
- 사이드바 진입은 배포 환경의 전체 route 프리페치와 Next.js 기본 링크 전환을 유지한다. 화면 내부 로컬 탭 상태와는 별개다.
