...
1. 핵심 이슈 카드 내부의 `확인 요청 현황`, 금주·차주 주요 이슈
2. 부서별 작성 모니터링 표
...
- 화면 내용 상단의 `확인요청현황`은 선택한 부서·화주 범위의 모든 미종결 요청을 주차와 관계없이 표시한다.
- 목록과 제목 상세는 부서자료의 `사업부 요청사항`과 같은 보드 및 `MeetingItemDetailDialog` 계약을 사용하며 요청 수정·삭제, 처리결과, 종결 상태를 즉시 반영한다.
