...
`/mini-game`은 활성 사용자가 업무 시스템 안에서 짧게 이용하는 `흰둥이의 산책` 캔버스 게임과 점수 랭킹 메뉴다. 모바일 `/mobile?view=game`에서는 별도의 세로형 Endless Shooter `하늘을 나는 흰둥이`를 제공한다. 흰둥이는 아래에서 위로 비행하고 적은 위에서 내려오며, 화면 좌측 하단 조이스틱과 우측 하단 필살기 버튼을 사용한다. 두 게임 모두 보고서, 승인, 권한 마스터와 업무 데이터 결합을 만들지 않는다.

일반 웹 사이드바에는 미니게임 메뉴를 노출하지 않는다. 모바일 앱 하단 메뉴의 미니게임은 계속 제공하며 기존 `/mini-game` 직접 경로와 점수·랭킹 계약은 호환을 위해 유지한다.

모바일 슈팅 게임의 다중 탄환 업그레이드는 5레벨마다 선택지에 나타나며 최대 3발까지만 확장된다. 게임오버 화면은 저장된 점수를 반영한 로그인 사용자의 현재 순위와 전체 TOP 10을 함께 표시한다.
