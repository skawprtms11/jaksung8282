...
export function MobileSettings({ profile, departmentName, clients }: {
  ...
  return <div className="space-y-3">
    <section className="border-y border-[#d9e7f7] bg-white px-4 py-5">
      <div className="flex items-center gap-3">
        <span className="inline-flex h-12 w-12 shrink-0 items-center justify-center rounded-full bg-[#e8f1ff] text-[#075be8]"><UserRound className="h-6 w-6" /></span>
    ...
    <section className="divide-y divide-slate-100 border-y border-[#d9e7f7] bg-white">
    ...
    <section className="border-y border-[#d9e7f7] bg-white">
      <div className="flex items-center gap-2 px-4 py-3"><UsersRound className="h-5 w-5 text-[#075be8]" /><div><h2 className="text-sm font-black">관리 가능한 화주</h2><p className="text-[11px] font-bold text-slate-500">현재 계정에 연결된 화주 목록입니다.</p></div><span className="ml-auto rounded-full bg-[#e8f1ff] px-2 py-1 text-[11px] font-black text-[#075be8]">{clients.length}</span></div>
...
function SettingRow({ icon: Icon, label, value }: { icon: typeof Building2; label: string; value: string }) {
  return <div className="flex items-center gap-3 px-4 py-3"><Icon className="h-5 w-5 shrink-0 text-slate-400" /><span className="text-xs font-black text-slate-500">{label}</span><span className="ml-auto max-w-[62%] truncate text-right text-sm font-black text-[#10223d]">{value}</span></div>;
}
