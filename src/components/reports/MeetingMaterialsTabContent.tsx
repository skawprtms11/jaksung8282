    ...
    <div className="grid w-full gap-3 xl:grid-cols-2">
      ...
      <section className="sketch-panel p-3">
        <div className="mb-3 flex min-h-[54px] items-center justify-between gap-3">
          ...
          <div className="flex shrink-0 items-center gap-1.5 rounded-2xl border border-[#d9e7f7] bg-[#f8fbff] px-2.5 py-1.5">
            <div className="w-24 shrink-0">
              <div className="mb-1 flex items-center justify-between gap-2"><span className="text-[10px] font-black text-slate-500">작성진행률</span><span className="text-[11px] font-black text-[#075be8]">{rate}%</span></div>
              <div className="h-2 overflow-hidden rounded-full bg-[#e8f1ff]"><span className="block h-full rounded-full bg-gradient-to-r from-[#0ea5e9] to-[#075be8]" style={{ width: `${rate}%` }} /></div>
            </div>
            {[{ label: "총건수", value: total, tone: "text-[#10223d]" }, { label: "완료건수", value: completed, tone: "text-[#075be8]" }, { label: "완료율", value: `${rate}%`, tone: "text-emerald-600" }].map((item) => (
          ...
          <table className="table-sticky w-full table-fixed text-left text-[13px]">
            ...
            <tbody>{data.departments.map((department) => {
              ...
              return <tr key={department.id} className="border-t border-slate-100">
                <td className="truncate px-2 py-1.5 text-[12px] font-black text-[#10223d]">{department.department_name}</td>
                <td className="px-2 py-1.5"><span className="font-black text-[#075be8]">{writtenClients}</span><span className="text-slate-400"> / {totalClients}</span></td>
    ...
    <div className="grid gap-4 xl:grid-cols-[1fr_460px]">
      ...
      <section className="sketch-panel p-4"><h2 className="section-doodle-title mb-3">물동량 요약</h2><TableShell>
        <table className="table-sticky w-full min-w-[420px] table-fixed text-left text-xs"><thead><tr><th className="px-3 py-2.5">주차</th><th className="px-2 py-2.5 text-right">입고</th><th className="px-2 py-2.5 text-right">출고</th><th className="px-3 py-2.5 text-right">합계</th></tr></thead>
          <tbody>{rows.map((row) => <tr key={row.name} className="border-t border-slate-100"><td className="px-3 py-2 font-black text-[#10223d]">{row.name}</td><td className="px-2 py-2 text-right font-bold">{row.inbound.toLocaleString("ko-KR")}</td><td className="px-2 py-2 text-right font-bold">{row.outbound.toLocaleString("ko-KR")}</td><td className="px-3 py-2 text-right font-black text-[#10223d]">{row.total.toLocaleString("ko-KR")}</td></tr>)}
            <tr className="border-t border-[#cfddec] bg-[#f5f9ff]"><td className="px-3 py-2.5 font-black">합계</td><td className="px-2 py-2.5 text-right font-black text-emerald-700">{total.inbound.toLocaleString("ko-KR")}</td><td className="px-2 py-2.5 text-right font-black text-blue-700">{total.outbound.toLocaleString("ko-KR")}</td><td className="px-3 py-2.5 text-right font-black">{total.total.toLocaleString("ko-KR")}</td></tr>
          </tbody></table>
      </TableShell></section>
    </div>
