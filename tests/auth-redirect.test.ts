...
describe("auth redirect normalization", () => {
  it("keeps local paths with search params and hashes", () => {
    ...
    expect(normalizeAuthRedirect("/mobile?view=department&department_id=team-1")).toBe("/mobile?view=department&department_id=team-1");
  });

  it("treats the mobile app as an authenticated route", () => {
    expect(protectedRoutes).toContain("/mobile");
  });
